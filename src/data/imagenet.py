...
def create_dataset_imagenet(dataset_dir, args, repeat_num=1, training=True):
    ...
    #     ]
    #     # transform_img += [rand_augment_transform(auto_augment, aa_params)]
    # transform_img_2 += [py_vision.AutoAugment(policy=py_vision.AutoAugmentPolicy.IMAGENET,
    #                                                interpolation=py_vision.Inter.BILINEAR,
    #                                                fill_value=0)]
    # transform_img_2 += [
    #         py_vision.ToTensor(),]
    
    # data_set = data_set.map(input_columns="image", num_parallel_workers=args.num_parallel_workers,
    #                         operations=transform_img_2)
    # iterator = data_set.create_dict_iterator(output_numpy =False)
